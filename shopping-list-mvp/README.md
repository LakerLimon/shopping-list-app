...
## Что важно для Android Java клиента

Android-клиенту нужно хранить `access` и `refresh` токены, а каждый защищённый запрос отправлять с заголовком:

```text
Authorization: Bearer ACCESS_TOKEN
```

Минимальные экраны Android можно подключать к этим endpoints через Retrofit + OkHttp.

## Что отложено за пределы MVP

- Firebase Cloud Messaging с реальными push-уведомлениями.
- QR-код генерация на backend.
- Полный офлайн-режим через Room.
- Аналитика расходов.
- Гибкие права участников.
- Сканирование штрихкодов.

В текущей версии уведомления реализованы как внутренние записи в таблице `shopping_notification`. Это сделано специально, чтобы MVP запускался без Firebase-проекта и секретных ключей.
